...
6. Launch a general-purpose sub-agent with `model: "sonnet"` (pattern-based task, analytical judgment not required). Pass it:
   ...
   - **Additional pattern categories to detect alongside the skill's four passes (pending the skill's own update via `/improve-skill`):**
     - **(a) Contrast-template overuse.** "Not X, but Y" / "X is Y, not Z" used as default paragraph rhythm rather than to sharpen a specific decision. Count contrast constructions; if ≥4 per 1,500-word section, rewrite lower-impact instances as direct statements. See prose-quality-standards Standard 10.
     - **(b) Abstract-noun stacking.** Three+ noun compounds before the main verb ("dominant cost nexus," "judgment-externalization tension," "sourcing-to-screening coupling point"). Test actor/verb rewrite; unless the compound is load-bearing document vocabulary (e.g., "Minimum Viable Service"), expand. See prose-quality-standards Standard 11.
     - **(c) Pseudo-maxim repetition.** Short aphoristic declarative sentences used as rhetorical habit ("Capability does not create entitlement," "Year 1 is the empirical test"). Budget: one maxim per section, reserved for the point the surrounding argument cannot carry. Rewrite excess maxims as ordinary sentences inside their paragraphs. See prose-quality-standards Standard 12.
     - **(d) Pivot closings.** Section-final sentences that gesture toward the next section's subject rather than landing this section's conclusion ("The question this opens is operational: what form the service takes…"). Remove or rewrite so the section closes on its own point. See prose-quality-standards Standard 13.
     - **(e) Flagged-vocabulary instances.** If the document's style-reference includes a Plain-Language Register / Flagged-Word Registry, scan the prose against it. Swap each flagged instance to its plain-English alternative unless the instance matches a named load-bearing carve-out in the registry.
