...
| Context pack session relevance | The context pack is tailored to the session's questions. Flag as **Moderate** if: (a) hypotheses appear that the session's directives do not test, (b) content map areas appear that the session does not investigate, or (c) prior findings are included that have no bearing on the session's questions. The context pack should orient the model toward what *this* session investigates, not describe the full section. |
